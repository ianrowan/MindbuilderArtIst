...
print("updated")
train = np.load("/home/ian/imgs_256_abstan.npy")

plt.imshow((train[200] + 1)/2)
plt.show()
...
def bias_var(shape, nam):
    init = tf.constant(0.00, shape=shape, name=nam+ "bias")
...
def Conv_Layer(input_, shape, name, str=1, pool=False):
    ...
    return h_conv
def deconv_layer(inp, shape, dim, stride, b):
...
def guassian_noise(input_layer,):
    noise = tf.random_normal(shape=tf.shape(input_layer), mean=0.0, stddev=.2, dtype=tf.float32)
...
def discrim(x, reuse=False):
    with tf.variable_scope("discriminator", reuse=reuse):
        x1 = guassian_noise(x)
        con_0 = Conv_Layer(x1, [5,5,3, 64], "conv_0", str=2)
...
def gen(z, batch, reuse=False):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        d_conv5 = deconv_layer(d_conv4, [5,5,3,64], end_size*32, stride=2, b=batch)
...
dis_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=labs, logits=d_logits))
gen_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=labs, logits=gen_logits))
...
with tf.control_dependencies(update_ops):
    train_dis = tf.train.AdamOptimizer(.0002,beta1=0.7).minimize(dis_loss, var_list=d_vars)
    train_gen = tf.train.AdamOptimizer(.0002, beta1=0.7).minimize(gen_loss, var_list=g_vars)
...
with tf.Session(config=config) as sess:
    ...
    write = tf.summary.FileWriter('/home/ian/GANs256_abstract', sess.graph)
    batch_size = 16
    try:
        for i in tqdm(range(80000)):
            labels_o = np.ones([batch_size,1])
            labels_z = np.zeros([batch_size,1])
            labelz = np.concatenate((labels_o, labels_z), axis=0)
            dis_in = next_batch(16,train)
            vector = np.random.uniform(-1, 1, size=[32, 100])
            dis_fake = generate.eval(feed_dict={z: vector})[:16]
            dis_in = np.concatenate((dis_in, dis_fake), axis=0)
            vector1 = np.random.uniform(-1, 1, size=[32, 100],)
            labels2 = np.ones([32, 1])
            if i%30 == 0:
                d_l = dis_loss.eval(feed_dict={images: dis_in, labs:labelz})
                g_l = gen_loss.eval(feed_dict={z:vector1, labs: labels2})
            ...
            #Train Discriminator
            train_dis.run(feed_dict={images: dis_in, labs:labelz})

            #Train Generator
            train_gen.run(feed_dict={z: vector1, labs: labels2})

            if i % 5000 == 0:
                saver.save(sess, "/home/ian/Server/GAN_HD.ckpt")
